...
var beget, criterion, factories, flatten, identity, implementsSqlFragmentInterface, prototypes, rawSql, _ref,
  __slice = [].slice;

criterion = require('criterion');

_ref = criterion.helper, identity = _ref.identity, beget = _ref.beget, implementsSqlFragmentInterface = _ref.implementsSqlFragmentInterface, rawSql = _ref.rawSql, flatten = _ref.flatten;
  ...
  sql: function(mohair, escape) {
    ...
    if (mohair._with != null) {
      ...
      parts = Object.keys(mohair._with).map(function(key) {
        return escape(key) + ' AS (' + rawSql(mohair._with[key]).sql(escape) + ')';
      });
      sql += parts.join(', ');
      sql += ' ';
    }
  ...
  params: function(mohair) {
    ...
    if (mohair._with != null) {
      Object.keys(mohair._with).forEach(function(key) {
        return params = params.concat(rawSql(mohair._with[key]).params());
      });
    }
...
factories.select = function() {
  ...
  return beget(prototypes.select, {
...
prototypes.insert = {
  sql: function(mohair, escape) {
    var escapedKeys, records, rows, sql, table;
    if (mohair._table == null) {
    ...
    rows = records.map(function(record) {
      var row;
      row = escapedKeys.map(function(key) {
...
factories.insert = function(recordOrRecords) {
  ...
  if (Array.isArray(recordOrRecords)) {
    ...
    return beget(prototypes.insert, {
...
factories.update = function(updates) {
  ...
  return beget(prototypes.update, {
    _updates: updates
  });
};
...
factories["delete"] = function() {
  return beget(prototypes["delete"]);
};
